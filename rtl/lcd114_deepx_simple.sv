module lcd114_deepx_simple (
    ...
    localparam int unsigned TEXT_SCALE = 6;
    ...
    localparam int signed SLIDE_MIN_X = -int'(TEXT_WIDTH);
    localparam int unsigned SLIDE_STEP = 8;
    ...
    function automatic logic text_pixel_on(input int signed x, input int signed y, input int signed text_x);
        ...
        int signed col_base_x;
        int signed row_base_y;
        logic [4:0] row_bits;
        begin
            text_pixel_on = 1'b0;

            for (int unsigned char_idx = 0; char_idx < 5; char_idx++) begin
                char_base_x = text_x + int'(char_idx * 6 * TEXT_SCALE);
                for (int unsigned row_idx = 0; row_idx < 7; row_idx++) begin
                    row_base_y = TEXT_Y + int'(row_idx * TEXT_SCALE);
                    row_bits = glyph_row(char_idx[2:0], row_idx[2:0]);
                    for (int unsigned col_idx = 0; col_idx < 5; col_idx++) begin
                        col_base_x = char_base_x + int'(col_idx * TEXT_SCALE);
                        if (row_bits[4 - col_idx]
                                && x >= col_base_x
                                && x < col_base_x + int'(TEXT_SCALE)
                                && y >= row_base_y
                                && y < row_base_y + int'(TEXT_SCALE)) begin
                            text_pixel_on = 1'b1;
                        end
                    end
                end
            ...
            unique case (init_state_q)
                ...
                DRAW_FRAME: begin
                    ...
                    if (pixel_cnt_q == PIXEL_COUNT[15:0]) begin
                        if (slide_x_q <= SLIDE_MIN_X) begin
