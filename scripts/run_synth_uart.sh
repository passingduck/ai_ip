...
yosys -p "read_verilog -sv rtl/uart_tx.sv rtl/uart_rx.sv rtl/uart_echo.sv fpga/tang9k_uart_top.sv; synth_gowin -top tang9k_uart_top -json build/tang9k_uart.json"

if command -v nextpnr-gowin >/dev/null 2>&1; then
  HOME="$NEXTPNR_HOME" nextpnr-gowin \
    --json build/tang9k_uart.json \
    --freq 27 \
    --write build/tang9k_uart_pnr.json \
...
gowin_pack -d GW1N-9C -o build/tang9k_uart.fs build/tang9k_uart_pnr.json
